...
1.  Write a function that calculates the density function of a Uniform continuous 
 ...
 value of `x` and parameters a and b and return the appropriate height of  the density function. For various values of `x`, `a`, and `b`, demonstrate that your function returns the correct density value. 
    a)  Write your function without regard for it working with vectors of data. 
        Demonstrate that it works by calling the function with a three times, once where $x< a$, once where $a < x < b$, and finally once where $b < x$.
        
```{r}
duniform <- function(x, a, b) {

  output <- ifelse(x<=b & a<=x, 1/(b-a), 0)
  return(output)
  
}

# a < x < b
value0 <- duniform(5, 4, 10)
value0

# x < a
value1 <- duniform(1, 2, 4)
value1

# b < x
value2 <- duniform(2, 4, 3)
value2
```
...
```{r}
        duniform <- function(x, a, b){
          ...
          for( i in 1:length ){  # Set the for loop to look at each element of x
            if( x[i]<=b & a<=x[i] ){  # What should this logical expression be?
              output[i] <- 1/(b-a)
            }else{
              output[i]<-0
            }
...
```{r}
  microbenchmark::microbenchmark( duniform( seq(-4,12,by=.0001), 4, 8), times=100)
```
The ifelse() version of the function was easier to write and was much faster than
than using a for loop to iterate through all of the members of x. 
...
6.  A common statistical requirement is to create bootstrap confidence intervals 
  ...
  for (i in 1:N) {
    ...
    row_indices <- sample(nrow(data), replace = TRUE)
    data_boot <- data[row_indices, ]
